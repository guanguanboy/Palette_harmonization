...
from models.gaussian_diffusion_eps import get_named_beta_schedule
...
class Network(BaseNetwork):
    ...
    def p_sample(self, y_t, t, clip_denoised=True, y_cond=None): #从y_t采样t时刻的重构值
        ...
        if True == self.is_ddim:
            out = self.spaced_dpm.ddim_sample(model=self.denoise_fn, x=y_t, t=t, clip_denoised=clip_denoised, denoised_fn=None, cond_fn=None,model_kwargs=model_kwargs)
    ...
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8): #采样过程，类似于IDDPM中的源码p_sample_loop
        ...
        y_t = default(y_cond, lambda: torch.randn_like(y_cond))
        ...
        for i in tqdm(reversed(range(0, self.time_step_respacing)), desc='sampling loop time step', total=self.time_step_respacing):
            ...
            if mask is not None:
                ...
                #pyr[-1] = y_t
                #y_restored = y_cond * (1. - mask) + mask*y_t
