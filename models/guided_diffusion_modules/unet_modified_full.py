...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.noise_level_emb = nn.Embedding(100, cond_embed_dim)
    ...
    def forward(self, x, mask, gammas):
        ...
        noise_level = self.estimation_net(x[:,0:3,:,:], mask) #torch.Size([3, 3, 64, 64])
        noise_level_mean = torch.mean(noise_level,dim=[1, 2,3]) #torch.Size([3])
        noise_level_mean_int = (noise_level_mean*100).to(torch.long)
