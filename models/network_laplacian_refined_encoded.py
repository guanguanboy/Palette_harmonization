...
class Network(BaseNetwork):
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None): #参数顺序，真实图片y_0，合成图片(条件图片y_cond)以及mask
        ...
        loss["loss"] += self.loss_fn(mask*target, mask*enlarged_output) + self.loss_fn(mask*target, mask*refined_output) + self.loss_fn(model_output_decoded*mask_down, y_cond_down*mask_down)    
